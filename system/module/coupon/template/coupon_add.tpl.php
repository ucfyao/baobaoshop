...
<script type="text/javascript" src="./statics/js/template.js" ></script>
		...
		<div class="content padding-big have-fixed-nav">
			<form action="<?php echo url('add')?>" method="post" name="coupon_add">
			<div class="form-box clearfix">
				<?php echo form::input('text', 'name', '', '优惠券名称：', '请输入优惠券名称', array(
					'datatype' => '*',
					'nullmsg' => '请输入优惠券名称',
					)
				); ?>
				<?php echo form::input('textarea', 'describe', '', '优惠券描述：', '请输入优惠券描述', array(
					'datatype' => '*',
					'nullmsg' => '请输入优惠券描述',
					)
				); ?>
				<?php echo form::input('text', 'num', '', '优惠券发放总量：', '请输入优惠券发放总量（请输入正整数或-1）', array(
					'datatype' => '*',
					'nullmsg' => '请输入发放总量',
					)
				); ?>
				<?php echo form::input('text', 'rules[discount]', '', '面值：', '请输入优惠券面值（请输入正数）', array(
					'datatype' => '*',
					'nullmsg' => '请输入面值',
					)
				); ?>
				<?php echo form::input('text', 'rules[condition]', '', '消费限制：', '请输入优惠券消费限制（请输入正数或-1）', array(
					'datatype' => '*',
					'nullmsg' => '请输入消费限制',
					)
				); ?>
				<?php echo form::input('text', 'receive_num', '', '领取数量限制：', '请输入会员领取该优惠券最多数量（请输入正整数）', array(
					'datatype' => 'n',
					'nullmsg' => '请输入最多领取数量',
					)
				); ?>
				<div class="type_time">
					<?php echo form::input('radio','type_time',1,'生效时间：', '请选择优惠券生效时间类型', array('items'=>array(1=>'活动时间范围',2=>'领取后几日失效'),'colspan'=>'3')); ?>
				</div>
				<div class="after">
					<?php echo form::input('text', 'time', '', '领取后失效天数：', '请输入会员领取该优惠券后几日失效（请输入正整数）'); ?>
				</div>
				<div class="range">
					<?php echo form::input('calendar', 'start_time', '', '开始时间：', '优惠券开始时间', array('format' => 'YYYY-MM-DD hh:mm')); ?>
					<?php echo form::input('calendar', 'end_time', '', '结束时间：', '优惠券结束时间', array('format' => 'YYYY-MM-DD hh:mm')); ?>
				</div>
				<div class="remind">
					<?php echo form::input('radio','remind',0,'是否开启提醒：', '请选择是否开启提醒', array('items'=>array(0=>'不开启',1=>'开启'),'colspan'=>'2')); ?>
				</div>
				<div class="remind_time">
					<?php echo form::input('text', 'remind_time', '', '到期提醒：', '请输入优惠券到期提醒天数（请输入正整数）'); ?>
				</div>
				<div class="type_remind">
					<?php echo form::input('checkbox','type_remind[]','','提醒类型：', '请选择优惠券提醒类型（开启后，请选择至少一种提醒类型）', array('items'=>array('短信'=>'短信','站内信'=>'站内信','邮件'=>'邮件'),'colspan'=>'3')); ?>
				</div>
				<div class="type_buy">
					<?php echo form::input('checkbox','type_buy','','购买类型：', '请选择优惠券购买类型', array('items'=>array(1=>'仅原价商品可用'),'colspan'=>'1')); ?>
				</div>
				<div class="type_use">
					<?php echo form::input('radio','type_use',1,'使用类型：', '请选择优惠券使用类型', array('items'=>array(1=>'全店商品',2=>'指定商品'),'colspan'=>'2')); ?>
				</div>	
			</div>
...
<script type="text/javascript">
	...
	var type_time = $('.remind input:checked').val();
	if(type_time == 0){
		$('.remind_time').hide();
		$('.type_remind').hide();
	}else{
	...
	$(function(){
		...
		var coupon_add = $("form[name=coupon_add]").Validform({
			ajaxPost:false
		})
