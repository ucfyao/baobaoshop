...
class goods_spu_service extends service {
	...
	public function goods_add($params){
		...
		runhook('before_goods_add',$result);
		return $result;
	}
	public function create_base($params,$cache){
		...
		if(!$params['catid']){
			$this->error = lang('goods_category_id_empty','goods/language');
			return FALSE;
		}
		if(!is_numeric($params['warn_number'])){
			$this->error = lang('inventory_warn_require','goods/language');
			return FALSE;
		}
		if(!is_numeric($params['sort'])){
			$this->error = lang('sort_require','goods/language');
			return FALSE;
		}
		if(!empty($cache)){
	 ...
	 */
	public function ajax_sn($params){
		if((int)$params['id'] < 1){
			$this->error = lang('_param_error_');
			return FALSE;
		}
