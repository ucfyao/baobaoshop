...
class attachment_service extends service {
	...
	public function _initialize() {
		...
		$this->notify_driver = $this->load->table('notify_driver');
	...
	public function get_attachemnt() {
		$result = array();
		$notifys = $this->load->table('notify_driver')->getField('code,config', TRUE);
	...
	public function config($vars, $code) {
		...
		if($this->notify_driver->find($code)) {
			$rs = $this->notify_driver->update($notify);
		}else{
			$rs = $this->notify_driver->add($notify);
		}
		if($rs === false) {
	...
	public function get($key = NULL){
		$result = array();
		$result = $this->load->table('notify_driver')->cache('attachment_enable',3600)->getField('code,name',true);
